...
class ModelMaker:
    ...
    def _domain_adaption(self,x):
        x = tf.keras.layers.Conv2D(128,kernel_size=2,strides=2,padding='valid',trainable=True,name="adaption_1")(x)
        x = tf.keras.layers.Conv2D(64,kernel_size=2,strides=2,padding='same',name="adaption_2")(x)
        x = tf.keras.layers.Conv2D(16,kernel_size=2,strides=2,padding='valid',name="adaption_3")(x)
        x = tf.keras.layers.Flatten(name="adaption_4")(x)
        x = tf.keras.layers.Dense(512,activation="relu",name="adaption_5")(x)
        x = tf.keras.layers.Dense(64,activation="relu",name="adaption_6")(x)
        x = tf.keras.layers.Dense(16,activation="relu",name="adaption_7")(x)
        x = tf.keras.layers.Dense(4,activation="softmax",name="adaption_8")(x)
        return x
    ...
    def create_models(self):
        ...
        training_outputs = [stage00_output,stage1_output, stage2_output, stage3_output, stage4_output, stage5_output, stage6_output]
        ...
        adapt_model = tf.keras.Model(inputs=adapt_input,outputs=env)
        return adapt_model,train_model, test_model
...
class ModelDatasetComponent:
    ...
    def place_training_labels(self, elem):
        ...
        if self.INCLUDE_MASK:
            ...
            inputs = image
        return inputs, (env,paf_tr, paf_tr, paf_tr, paf_tr, kpt_tr, kpt_tr)  # this should match the model outputs, and is different for each model
