...
class ModelMaker:
    ...
    def _domain_adaption(self,x):
        x = tf.keras.layers.Conv2D(128,kernel_size=2,strides=2,padding='valid',trainable=True)(x)
        x = tf.keras.layers.Conv2D(64,kernel_size=2,strides=2,padding='same')(x)
        x = tf.keras.layers.Conv2D(16,kernel_size=2,strides=2,padding='valid')(x)
    ...
    def create_models(self):
        ...
        env = self._domain_adaption(stage00_output)
        # stage 0 2conv)
        ...
        training_outputs = [env,stage1_output, stage2_output, stage3_output, stage4_output, stage5_output, stage6_output]
        ...
        # adapt_model = tf.keras.Model(inputs=input_tensor,outputs=env)
