...
class DatasetTransformer:
    def __init__(self, config):
        ...
        self.feature_description = {
                ...
                'environment' : tf.io.FixedLenFeature([], tf.string),
                }
        self.init_grid()
    ...
    def read_tfrecord(self, serialized_example):
        ...
        environment = tf.io.parse_tensor(parsed['environment'], tf.float32)
        environment = tf.ensure_shape(environment,4)
        # environment = parsed['environment']
        # environment = tf.RaggedTensor.from_tensor(environment).to_tensor()

        return {"id": idd, "image_raw": image_raw, "size": size, "kpts": kpts, "joints": joints, "mask": mask,"environment":environment}
    ...
    def open_image(self, elem):
        ...
        converts raw jpeg stored as bytes form to array and resizes to config determined size,
         also converts blackwhite saveimages to 3 channels"""
    ...
    def image_only_augmentation(self, elem):
        ...
        randomly changes the color,contrast,brightness of the input saveimages"""
        image = elem["image"]
        # adjust contrast
