...
def encode_example(idd, image_raw, size, kpts, joints, mask):
    ...
    #laboratory 1000
    #conferenceroom 0100
    environment = np.array((0,1,0,0)).astype(np.float32)

    environment=tf.constant(environment)
    environment=tf.io.serialize_tensor(environment).numpy()
    ...
    feature = {
        ...
        'environment': bytes_feature(environment)
...
def create_all_joints(all_keypts, config):
    ...
    all_joints = [create_joints(x) for x in all_keypts]  # for each person epsilon = backend_config.epsilon
...
def coco_to_TFrecords(keypoint_annotations_file, transformed_annotations_file, config,type):
    ...
    # imgIds.sort()
    print("Found %d saveimages" % len(imgIds))
    nowt = int(time.time() / 100)
    files_path = transformed_annotations_file + type+'-' + str(nowt) + "-conferenceroom"+"-{:03}.tfrecords"
    with FileSharder(tf.io.TFRecordWriter, files_path, config.IMAGES_PER_TFRECORD) as writer:
        for img_id in imgIds:
            ...
            try:
                ...
                total_mask = cv2.imread(img_path)[...,0]
                total_mask = np.where(total_mask > 0.01,1,0).astype(np.int16)
                total_mask = cv2.resize(total_mask, (config.LABEL_HEIGHT, config.LABEL_WIDTH))
...
if __name__ == "__main__":
    ...
    coco_to_TFrecords(cfg.VALID_ANNS, cfg.VALID_TFRECORDS, cfg,"validation")
    coco_to_TFrecords(cfg.TRAIN_ANNS, cfg.TRAIN_TFRECORDS, cfg,"training")
    print("dataset for train transformed")
